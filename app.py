...
if st.session_state.get('workflow'):
    ...
    def build_script(workflow,module_filename=None):
        ...
        func=getattr(custom,"'+step['custom_function']+'")
        return func(**(context.get("'+fn_name+'_params",{})))')
            if step['loop']:
    ...
    st.header('Notes')
    st.write('This prototype generates a script skeleton based on inputs. Customize the generated script in the editor and download or save templates for reuse.')
